    ...
    if tmp:
        if role == "Сотрудник":
            ...
            if role =='Подчиненный директора':
                await ID.pr_id.set()
                await message.answer(
                    f"{vipo}\n{bezpotv}\n{proc}\n{nfulf}\n\nДля изменения статуса проблемы, напишите ее номер:")
            else:
                await vipol.vip.set()
                await message.answer(f"{vipo}\n{bezpotv}\n{proc}\n{nfulf}\n\nДля подтверждения проблемы, напишите ее номер")
    ...
    try:
        ...
        if tmp[1] != 2:
            if int(pr_ident) != tmp[0]:
                ...
                await message.answer("Выберите статус проблемы:", reply_markup=keyboard)
        else:
            buttons = ["Назад"]
            keyboard.add(*buttons)
